...
function updateBar(){
	...
	bar2Height += barMin;
	//base card value
	var baseCardValue = Math.floor(milesVal[1].value*12);//bar 2 card value
	var bar2CardValue = Math.floor(milesVal[2].value*12);
	if(milesVal.length==3){
		...
		var bar3CardValue = Math.floor(milesVal[3].value*12);
		$(".bar1").animate({height:bar1Height},1);
		$(".bar1 .bartext p").html(baseCardValue);
		//
